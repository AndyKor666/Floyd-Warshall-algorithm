...
void FrontHvyli(int** matr, int n, int P) {
	const int t = INT_MAX;
	int* dist = new int[n];
	bool* byval = new bool[n];
	for (int i = 0; i < n; i++) {
		dist[i] = t;
		byval[i] = false;
	}
	...
	while (front < rear) {
		int i = queue[front++];
		for (int j = 0; j < n; j++) {
			if (matr[i][j] != 0 && matr[i][j] != t && !byval[j]) {
				dist[j] = dist[i] + 1;
				byval[j] = true;
				queue[rear++] = j;
			}
...
int main() {
	...
	cout << ">> Введіть початкову вершину для хвилі (1 - " << n << "): ";
	cin >> s;
	cout << "----------------------------------------\n";
	FrontHvyli(matr, n, s - 1);
	cout << "----------------------------------------\n";
	cout << "========================================\n\n";
	free(graph);
	Delete(matr, n);
